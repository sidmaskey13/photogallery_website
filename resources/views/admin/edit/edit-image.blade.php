...
@section('content')
    <div class="container">
        ...
        <form method="POST" action="{{url('edit-image/'.$edit_image->id)}}" enctype="multipart/form-data">
            {{@csrf_field()}}
            <div class="row">
                <div class="col-md-12">
                    <label for="title">Title:</label>
                    <input type="text" class="form-control" id="title" placeholder="Enter Media Title" name="title" value="{{$edit_image->title}}" required>
                </div>
            </div>
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <select name="tag_test[]"  id="input-tags" class="demo-default" multiple>
                        @foreach($tags as $t)
                            <option value="{{$t->name}}" @foreach ($edit_image->tag as $m)
                ...
                <div class="col-md-6">
                    ...
                    <div class="category-box">
                        <div class="row">
                            @foreach ($categories as $c)
                                <div class="col-md-4">
                                    <input type="checkbox" value="{{$c->id}}" name="category[]" @foreach ($edit_image->category_media as $u)
                                ...
                                @foreach($license_modify as $f)
                                    ...
                                    <div class="col-md-12">
                                        <input type="radio" name="license_modify" value="{{$f->id}}"
                                               @foreach ($edit_image->license as $l)
                    ...
                    <input type="text" class="form-control" id="author_name" placeholder="Enter Author Name"
                           name="author_name" value="{{$edit_image->author_name}}" required>
                </div>
            </div>

            <input type="hidden" id="latitude" name="latitude" value="{{$edit_image->latitude}}" class="form-control">
            <input type="hidden" id="longitude" name="longitude" value="{{$edit_image->longitude}}" class="form-control">
            <div class="row">
                <div class="col-md-8">
                    <label for="info">Info:</label>
                    <textarea class="info" name="info" id="info" rows="5">{{$edit_image->info}}</textarea>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="row">
                    @foreach($edit_image->all_image as $s)
                ...
                {{--<input type="hidden" name="oldimage" value="{{$edit_image->media}}">--}}
            </div>
            <br>
            <div>
