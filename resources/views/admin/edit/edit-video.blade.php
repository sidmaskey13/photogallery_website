...
@section('content')
    <div class="container">
        ...
        <form method="POST" action="{{url('edit-video/'.$edit_video->id)}}" enctype="multipart/form-data">
            {{@csrf_field()}}
            <div class="row">
                <div class="col-md-12">
                    <label for="title">Title:</label>
                    <input type="text" class="form-control" id="title" placeholder="Enter Video Title" name="title" value="{{$edit_video->title}}" required>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    ...
                    <select name="tag_test[]" id="input-tags" class="demo-default" multiple>
                        @foreach($tags as $t)
                            <option value="{{$t->name}}" @foreach ($edit_video->tag_video as $m)
                ...
                <div class="col-md-6">
                    ...
                    <div class="category-box">
                        <div class="row">
                            @foreach ($categories as $c)
                                <div class="col-md-4">
                                    <input type="checkbox" value="{{$c->id}}" name="category[]" @foreach ($edit_video->category_media as $u)
                                ...
                                @foreach($license_modify as $f)
                                    ...
                                    <div class="col-md-12">
                                        <input type="radio" name="license_modify" value="{{$f->id}}"
                                               @foreach ($edit_video->license as $l)
                    ...
                    <input type="text" class="form-control" id="author_name" placeholder="Enter Author Name"
                           name="author_name" value="{{$edit_video->author_name}}" required>
                </div>
            </div>

            <input type="hidden" id="latitude" name="latitude" value="{{$edit_video->latitude}}" class="form-control">
            <input type="hidden" id="longitude" name="longitude" value="{{$edit_video->longitude}}" class="form-control">
            <div class="row">
                <div class="col-md-8">
                    <label for="info">Info:</label>
                    <textarea class="info" name="info" id="info" rows="5">{{$edit_video->info}}</textarea>
                </div>
            </div>
            <div class="form-group">
                <label for="image">Old Video:</label>
                <div class="row">
                    @foreach($edit_video->all_video as $s)
                        <div class="col-md-3" id="old-img-{{$s->id}}">
                            <div class="gallery-container">
                                <div id="myBtn_{{$s->id}}" onclick="showModal({{$s->id}})" class="gallery-single-container">
                                    <video src="{{url($s->video_file)}}" height="100% " width="100%" type="video/mp4" controls/>
                                </div>
                                {{--<form method="POST" action="{{URL::to('/edit-delete/'.$s->id)}}">--}}
                ...
                <label for="image">New Video:</label>
                ...
                <input type="hidden" name="oldimage" value="{{$edit_video->video}}">
            </div>
            <br>
            <div>
